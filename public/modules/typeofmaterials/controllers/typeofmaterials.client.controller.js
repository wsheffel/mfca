...
angular.module('typeofmaterials').controller('TypeofmaterialsController', 
	...
	function($scope, $http, $stateParams, $location, Authentication, Typeofmaterials, _, Articles, $filter) {
		...
		//isEdit for typeofmaterials items
		...
		$scope.loadItems = function(){
			if(_.isEmpty($scope.list_of_materials) && $scope.isEditItems === true){
				$scope.list_of_materials = $scope.typeofmaterial;
		...
		// Edit Items for typeofmaterials
		...
		$scope.update = function() {
			if(!_.isUndefined($scope.typeofmaterial) &&
					_.size($scope.typeofmaterial) > 0){				
				_.forEach($scope.typeofmaterial, function (saveObj){
