...
angular.module('materiallosses').controller('MateriallossesController', 
	...
	function($scope, $stateParams, $location, Authentication, Materiallosses, _) {
		...
		$scope.create = function() {
			...
			console.log($scope.total_percentage);
			...
			console.log(angular.toJson(materialloss));

			// Redirect after save
			materialloss.$save(function(response) {
				$location.path('materiallosses/' + response._id);
		...
		$scope.remove = function(materialloss) {
			if ( materialloss ) { 
				...
				for (var i in $scope.materiallosses) {
					if ($scope.materiallosses [i] === materialloss) {
						$scope.materiallosses.splice(i, 1);
					}
				}
			} else {
